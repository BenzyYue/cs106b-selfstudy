...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    if(str_copy.size() < kGramLength){
        return map;
    }

    while (str_copy.size() >= kGramLength) {
        string to_be_put = str_copy.substr(0,  kGramLength);
        if(map.containsKey(to_be_put)){
            map[to_be_put] += 1;
        }
        else{
            map.put(to_be_put, 1);
...
Map<string, double> normalize(const Map<string, double>& input) {
    double sum = 0;
    Map<string, double> map_copy = input;
    for(const string& key : map_copy){
        sum = sum + (map_copy[key] * map_copy[key]);
    }
    if(sum == 0){
        error("Invalid input");
    }
    for(const string& key : map_copy){
        map_copy[key] = map_copy[key] / sqrt(sum);
    }
    return map_copy;
}

Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    PriorityQueue<string> map_queue;
    Map<string, double> top_k_map;
    if((source.size() < numToKeep) || (source.size() == 0)){
        return source;
    }
    if(numToKeep == 0){
        return top_k_map;
    ...
    if(numToKeep < 0){
        error("Invalid number to keep");
    }
    for(const string& key : source){
        map_queue.enqueue(key, -source[key]);
    }
    for(int i = 0; i < numToKeep; i++){
        string elem = map_queue.dequeue();
        top_k_map.put(elem, source[elem]);
    }
    return top_k_map;
}
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    double sim = 0;
    for(const string& keyL : lhs){
        for(const string& keyR : rhs){
            if(keyL.compare(keyR) == 0){
                sim = sim + lhs[keyL] * rhs[keyR];
            }
        }
    }
    return sim;
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    if(corpora.size() == 0){
        error("No corpora is provided.");
    }
    PriorityQueue<string> map_queue;
    double sim;
    for(const Corpus& corpus : corpora){
        sim = -cosineSimilarityOf(textProfile, corpus.profile);
        map_queue.enqueue(corpus.name, sim);
    }
    return map_queue.dequeue();
}
