...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
                            ...
                            double height) {
    int grid_size_row = terrain.numRows();
    int grid_size_col = terrain.numCols();
    Queue<GridLocation> queue_BFS;
    Grid<bool> floodedRegions(grid_size_row, grid_size_col);

    for(const GridLocation& source : sources){
        queue_BFS.enqueue(source);
        floodedRegions[source.row][source.col] = true;
    }

    while(!queue_BFS.isEmpty()){
        GridLocation grid = queue_BFS.dequeue();
        Queue<GridLocation> queue_temp;

        GridLocation grid_left;
        grid_left.row = grid.row - 1;
        grid_left.col = grid.col;
        queue_temp.enqueue(grid_left);

        GridLocation grid_right;
        grid_right.row = grid.row + 1;
        grid_right.col = grid.col;
        queue_temp.enqueue(grid_right);

        GridLocation grid_down;
        grid_down.row = grid.row;
        grid_down.col = grid.col - 1;
        queue_temp.enqueue(grid_down);

        GridLocation grid_up;
        grid_up.row = grid.row;
        grid_up.col = grid.col + 1;
        queue_temp.enqueue(grid_up);

        while(!queue_temp.isEmpty()){
            grid = queue_temp.dequeue();
            if(floodedRegions.inBounds(grid.row, grid.col) && terrain[grid.row][grid.col] < height && floodedRegions[grid.row][grid.col] == false){
                floodedRegions[grid.row][grid.col] = true;
                queue_BFS.enqueue(grid);
            }
        }
    }
